...
public class ZephyrProtocol implements ReceivedListener {
	...
	// This is a array of objects that implement the ZephyrPacketListener
	// interface.
	...
	private void OnZephyrPacket(ZephyrPacketArgs packet) {
		Vector<ZephyrPacketListener> handler = (Vector<ZephyrPacketListener>) eventSubscribers
				.clone();
		Iterator<ZephyrPacketListener> iter = handler.iterator();
		while (iter.hasNext()) {
			// This loop looks to process all objects that implement the
			// Received Packet interface and
	...
	// Constructor for ZephyrProtocol. This instantiates the BH to send GP and
	// ECG Packets
	public ZephyrProtocol(BTComms comms, PacketTypeRequest ReqPcktType) {
		_packet = new ZephyrPacket();

		_comms = comms;
		// Ties the ZephyrProtocol object's Received Method to respond to the
		// ReceivedPacket event
	...
	public boolean SetGeneralPacket(boolean activate) {
		...
		// Creates and Adds GeneralPacketListener object to the list of event
		// subscribers to respond to the received General Packet Listener event
		...
		System.out.println(String.format("Set General Packet status %s",
				task.ack));
	...
	public boolean SetBreathingPacket(boolean activate) {
		...
		// Remove Listening for ECG Ack Packet
		removeZephyrPacketEventListener(task);
		System.out.println(String.format("Set Breathing Packet status %s",
				task.ack));
	...
	public boolean SetRtoRPacket(boolean activate) {
		...
		// Remove Listening for ECG Ack Packet
		removeZephyrPacketEventListener(task);
		System.out.println(String.format("Set R to R Packet status %s",
				task.ack));
	...
	public boolean SetAccelerometerPacket(boolean activate) {
		class SetAccelerometerListener implements ZephyrPacketListener {
			...
			public void ReceivedPacket(ZephyrPacketEvent eventArgs) {
				// TODO Auto-generated method stub
				if (ACCELEROMETER_PACKET_ACK == eventArgs.getPacket()
						.getMsgID()) {
					if (eventArgs.getPacket().getStatus() == 0x06)
						ack = true;
					synchronized (this) {
		...
		// Remove Listening for ECG Ack Packet
		removeZephyrPacketEventListener(task);
		System.out.println(String.format("Set Accelerometer Packet status %s",
				task.ack));
	...
	public boolean SetSummaryDataPacket(boolean activate) {
		...
		removeZephyrPacketEventListener(task);
		System.out.println(String.format("Set Summary Data Packet status %s",
				task.ack));
	...
	public boolean SetLoggingDataPacket(boolean activate) {
		...
		// Remove Listening for Summary Ack Packet
		removeZephyrPacketEventListener(task);
		System.out.println(String.format("Set Logging Data Packet status %s",
				task.ack));
	...
	public void Received(ReceivedEvent eventArgs) {
		...
		// This function parses the byte by byte until there are no more bytes
		// to be processed
		while (iter.hasNext()) {
			ZephyrPacketArgs msg = null;
			try {
				// This function parses the byte by byte looking for a pattern
				// of
				// a STX, Valid CRC, inconsistency in packet length, and ETX. If
