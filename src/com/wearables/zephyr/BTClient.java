...
public class BTClient extends Thread {
	...
	public void IsValidBlueToothAddress() {
		_isValidBlueToothAddress = _adapter
				.checkBluetoothAddress(_connectionString);
	}

	private Vector<ConnectedListener<BTClient>> eventSubscribers = new Vector<ConnectedListener<BTClient>>();

	/*
	 * This is an array of objects that implement the ConnectedListener
	 * interface. Right now the only class that implements it is
	 * ConnectListenerImpl class and it implements the Connected method
	 */
	private void OnConnected() {
		@SuppressWarnings("unchecked")
		// Create a new Vector with exactly same elements as eventSubscribers
		Vector<ConnectedListener<BTClient>> handler = (Vector<ConnectedListener<BTClient>>) eventSubscribers
				.clone();
	...
	public void removeConnectedEventListener(
			ConnectedListener<BTClient> listener) {
	...
	public BTClient(BluetoothAdapter adapter, String connectionString) {
		...
		try {
			// Create a Secure RFComm Bluetooth socket for secure outgoing
			// connection with device
			// using a given
			// "UUID--using well known UUID SPP suggested by Android??"
			_btStream = _device.createRfcommSocketToServiceRecord(MY_UUID);
			if (_btStream != null) { // Attempt to connect to a remote
										// device...Needs an exception check
										// here to
										// see if it fails
				_btStream.connect();
				_isConnected = true;
	...
	private void StartCommunication() {
		...
		// Calls Connected method which calls ReceivedPacketMethod that prints
		// information on the phone's text box based on the message type
		OnConnected();
	}
